...
class Tree(object):
    ...
    def insert(self, newData):
        if self.root.data == None:
            ...
            self.__insert(self.root.parent, self.root, newData)

    def __insert(self, parent, root, newData):
        if newData < root.data:
            if root.left:
                self.__insert(root, root.left, newData)
            else:
                root.left = Node(newData)
                root.parent.left.left = Node(newData)
        elif newData > root.data:
            if root.right:
                self.__insert(root ,root.right, newData)
            else:
                root.right = Node(newData)
                root.parent.right.left = Node(newData)
